...
{//Please note that the DataGenerator class will not likely be used for a software company's solution. Instead the information below would come from a database
    public class DataGenerator
    {
        ...
        public BankcardTransactionPro SetBankCardTxnData(BankCardProProcessingOptions _BCPPO)
        {
            ...
            {//Keyed Transaction
                TxnData.EntryMode = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.EntryMode.Keyed;
            ...
            {//Swiped Transaction
                ...
                foreach (string track in TrackProcessingOrder)
                {
                    if (track == "Track1")
                        ...
                        {
                            TxnData.EntryMode = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.EntryMode.TrackDataFromMSR;
                            break;
                        }
                    if (track == "Track2")
                        if (helper.validateTrackData(ref BCtransaction, TFMSRS.Track2Data))
                        {
                            TxnData.EntryMode = schemas.ipcommerce.com.Ipc.General.WCF.Contracts.Common.External.Txn.EntryMode.Track2DataFromMSR;
                            break;
                        }
                    if (track == "Keyed")
                    {
        ...
        public void SandboxTestCardData()
        {
            if (cardType == TypeCardType.Visa)
            {
                ...
                TrackDataFromMSRVal = "%B4111111111111111^IPCOMMERCE/TESTCARD^13121010454500415000010?;4111111111111111=13121010454541500010?";
            }
            else if (cardType == TypeCardType.MasterCard)
            {
                pan = "5454545454545454";
                TrackDataFromMSRVal = "%B5454545454545454^IPCOMMERCE/TESTCARD^1312101013490000000001000880000?;5454545454545454=13121010134988000010?";
            }
            else if (cardType == TypeCardType.AmericanExpress)
            {
                pan = "371449635398456";
                TrackDataFromMSRVal = "%B371449635398456^IPCOMMERCE/TESTCARD^1312060523319?;371449635398456=1312060523319?";
            }
            else if (cardType == TypeCardType.Discover)
            {
                pan = "6011000995504101";
                TrackDataFromMSRVal = "%B6011000995504101^IPCOMMERCE/TESTCARD^13121011000627210201?;6011000995504101=13121011000627210201?";
            }
            else if (cardType == TypeCardType.PrivateLabel)
            {
                pan = "8818889876543211";
                TrackDataFromMSRVal = ";8818889876543211=13121014764094900001?";
            }
            else
            {
                pan = "";
